...
function add_files {
  ...
  if [[ ${GAPPS_TYPE} = "aroma" ]]; then
      ...
      cp -rvf ${rom_root}/${GAPPS_ZIP}/META-INF/com/google/android/aroma/scripts ${tmp_root}/META-INF/com/google/android/aroma/ >> ${build_log} 2>&1
  fi

  echo ">> Add FreedomOS wallpapers" 2>&1 | tee -a ${build_log}
  ...
  if [[ ! -z $PATCH_FIRMWARE ]]; then
    ...
    mkdir -p ${tmp_root}/firmware-update/${PATCH_FIRMWARE}/ | tee -a ${build_log}
    cp -rvf ${assets_root}/firmware-update/$DEVICE/${PATCH_FIRMWARE}/firmware-update ${tmp_root}/firmware-update/${PATCH_FIRMWARE}/ >> ${build_log} 2>&1
  fi
}
